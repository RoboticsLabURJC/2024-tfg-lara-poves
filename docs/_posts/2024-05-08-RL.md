...
---
Cada algoritmo *Actor-Critic* se compone de dos elementos que aprenden de manera conjunta: ***actor***, encargado de aprender la política óptima (***π***), y el ***critic***, responsable de aprender la función de valor (***Q***) y proporcionar una señal de refuerzo a la política.
...
- Función objetivo: J(πθ) = Et~πθ[Aπθ] (E = esperanza) 
- Para maximizar esta función objetivo, aplicamos la técnica de ascenso por gradiente: ∇θJ(πθ) = Et[Aπθ ∇θ log πθ(at|st)]
