...
last_modified_at: 2024-05-15T09:08:00
categories:
  - Blog
tags:
...
#### Algoritmo DQN
```python
 for e in range(MAX_EPISODES): # Número de epiosodios
  env.reset
  s = env.get_state()

  for m in range(MAX_STEPS): # Número de pasos
    a = select_action(s, θ) # Seleccionar una acción según la política actual
    ...
    for b in range(B): # Número de lotes por step
      ...
      for u in range(U): # Actualizaciones por lote
        for i in range(N): # Número de experiencias por lote
          s, a, r, s_next = b[i]
          y = r + gamma * max(Qφ(s_next, max(Qθ(s_next, a_next)))) 
