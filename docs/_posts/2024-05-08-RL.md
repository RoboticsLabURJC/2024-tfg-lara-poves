...
last_modified_at: 2024-05-30T11:42:00
categories:
  - Blog
tags:
...
<p style="text-align:center; font-weight:bold;">
  newQ(s, a) = (1 - α) * Q(s, a) + α[R(s, a) + γ[maxQ'(s', a') - Q(s, a)]]
</p>
...
for m in range(MAX_STEPS): # Número de pasos (interacciones con el entorno)
  ...
  for b in range(B): # Número de lotes por step
    ...
    for u in range(U): # Actualizaciones por lote
      ...
      θ = (1 - α) * θ + α * ∇θ * L(θ) # Actualizar los pesos de la red principal
...
Para elegir la siguiente acción usaremos la **función de ventaja (A)**, no nos basamos únicamente en la función de valor (Q, *θA*), sino que la combinamos con la acción promedio según la política actual (V, *θC*). De esta manera, la ventaja nos cuantifica cuánto mejor o peor es una acción en comparación con la media.
<p style="text-align:left; font-weight:bold;">
...
- Para maximizar esta función objetivo, aplicamos la técnica de ascenso por gradiente: \(\nabla_{\theta} J(\pi_{\theta}) = \mathbb{E}_{t}[A^{\pi_{\theta}}_{t} \nabla_{\theta} \log \pi_{\theta}(a_{t}|s_{t})]\)
...
Q^π(s_t, a_t) ≈ r_t + γr_{t+1} + γ^2r_{t+2} + ... + γ^n r_{t+n} + \hat{V}^π(s_{t+n+1})

A_{\text{NSTEP}}^π(s_t,a_t) = Q^π(s_t,a_t) - V^π(s_t) ≈ r_t + γr_{t+1} + γ^2r_{t+2} + ... + γ^n r_{t+n} + γ^{n+1} \hat{V}^π(s_{t+n+1}) - \hat{V}^π(s_t)
...
- \(V^{\pi}_{\text{tar}}(s) = r + \hat{V}^{\pi}(s'; \theta) = r_t + \gamma r_{t+1} + \ldots + \gamma^n r_{t+n} + \gamma^{n+1} \hat{V}^{\pi}(s_{t+n+1})\) (*n-steps*)
- Monte Carlo: \(V^{\pi}_{\text{tar}}(s) = A^{\text{GAE}\pi}(s_t, a_t) + \hat{V}^{\pi}(s_t)\) (GAE)
