...
last_modified_at: 2024-06-11T12:57:00
categories:
  - Blog
tags:
...
- V<sub>tar</sub>π<sup>π</sup>(s) = r + V<sup>π</sup>(s';θ) = r<sub>t</sub>​ + γ r<sub>t + 1</sub> ​+ γ<sup>2</sup> r<sub>t + 2</sub> ​+ … + γ<sup>n</sup> r<sub>t + n</sub> + V<sup>π</sup>(s<sub>t + n + 1</sub>) (*n-steps*)
- Monte Carlo: V<sub>tar</sub>π<sup>π</sup>(s) = A<sub>GAE</sub><sup>π</sup>(s<sub>t</sub>​, a<sub>t</sub>​) + V<sup>π</sup>(s<sub>t</sub>​) (GAE)
...
<p style="text-align:center; font-weight:bold;">
  J<sup>CLIP</sup>(θ) = E<sub>​t</sub>​[min(r<sub>​t</sub>​(θ) * A<sub>​t</sub>​, clip(r<sub>​t</sub>​(θ), 1−ϵ, 1+ϵ) * A<sub>​t</sub>​)]
</p>
ϵ es el hiperparametro que define el vecindario de recorte, el seguento termino limita el valor de JCPIJCPI​ a estar entre (1−ϵ)At(1−ϵ)At​ y (1+ϵ)At(1+ϵ)At. Cuando rt(θ)rt​(θ) está dentro del intervalo [1−ϵ,1+ϵ][1−ϵ,1+ϵ], ambos términos dentro del min⁡(⋅)min(⋅) son iguales. Este objetivo previene actualizaciones de parámetros que podrían causar cambios grandes y arriesgados en la política πθπθ, es decir, la politica antigua y nueva son muy diferentes​. Estos calculos son muchos menos costosos que los de la aneterior tecnica y obtiene un mejor rendimiento, por ello, esta es la que gneralmente s eusa.

*ϵ* es el hiperparámetro que define el vecindario de recorte, limita el valor de J<sup>CPI​</sup> entre [(1−ϵ)At<sub>​t</sub> y (1+ϵ)A<sub>​t</sub>]​. Cuando r<sub>​t</sub>(θ) está dentro del intervalo [1−ϵ, 1+ϵ], ambos términos dentro del min⁡(⋅) son iguales. Este objetivo previene actualizaciones de parámetros que podrían causar cambios grandes y arriesgados en la política​. Además, los cálculos son mucho menos costosos que los de la técnica anterior y obtienen un mejor rendimiento, por lo que generalmente es la estrategia utilizada.
...
Gym es una biblioteca de Python desarrollada por OpenAI que proporciona una variedad de entornos para aplicar algoritmos de aprendizaje por refuerzo, la usaremos para familiarizarnos con la librería *stable_baselines3* y poner en práctica los conocimientos sobre *deep* RL. Necesitamos instalar las siguientes bibliotecas:
```bash
...
Hemos desarrollado un programa en Python para crear y entrenar un modelo diseñado para tres entornos específicos de Gym. El parámetro *log interval* es un opcional y por defecto es 1. Los resultados pueden visualizarse directamente mediante el comando *tensorboard*. En algunos casos se pueden normalizar los estados y/o recompensas para que el entrenamiento sea más estable y acelerar su convergencia. Un ***rollout*** abarca un episodio y los datos recopilados durante el mismo (duración, recompensa y ratio de exploración).
```bash
...
La avarianza es la variabilidad en las recompensas de una partida a otra. La gráfica de *explained_variance* en la sección de *train* representa cuánto de esta variabilidad en las recompensas puede ser explicada por el modelo. Un valor cercano a 1 significa que el modelo puede explicar la mayoría de la variabilidad en las recompensas, esto sugiere que el modelo está aprendiendo efectivamente. Sin embargo, es importante tener en cuenta que siempre habrá cierto grado de azar involucrado. A medida que el modelo comienza a converger, las variaciones en las recompensas serán muy pequeñas pero inexplicables (*explained_variance* menor o igual a 0).
