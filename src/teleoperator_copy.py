...
def main(args):
    ...
    world, _ = configcarla.setup_carla(name_world='Town04', syn=False, port=args.port)
    screen = configcarla.setup_pygame(size=(configcarla.SIZE_CAMERA * 2, configcarla.SIZE_CAMERA), 
                                      name='Teleoperator')

    # Add Ego Vehicle
    vehicle_transform = carla.Transform(carla.Location(x=-410, y=32, z=0.1), carla.Rotation(yaw=-137))
    spectator = configcarla.center_spectator(transform=vehicle_transform, world=world, pitch=-90, height=150)
