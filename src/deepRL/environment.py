...
class CarlaBase(gym.Env, ABC):
    ...
    def _swap_ego_vehicle(self):
        ...
        self._camera = self._sensors.add_camera_rgb(transform=transform, seg=self._seg, lane=True,
                                                    ...
                                                    size_rect=(SIZE_CAMERA, SIZE_CAMERA), check_other_lane=True,
        ...
        if self._passing or self._sim: 
            ...
            # Front vehicle
            if self._passing and (not isinstance(self, CarlaOvertaken) or self._retrain):
    ...
    def step(self, action:np.ndarray):
        ...
        try:
            ...
            if self._id == 0:
                ...
                finish_ep = abs(loc.x + 417) <= 3 and abs(loc.y - 34) <= 3
            elif self._id == 9:
                finish_ep = abs(loc.x + 425) <= 3 and abs(loc.y + 37) <= 3
            elif self._id == 10:
        ...
        if terminated and self._train:
            ...
            self._writer_csv_train.writerow([self._count_ep, self._total_reward, self._count, finish_ep,
                                             ...
                                             self._mean_vel / self._count, self._count_laser, False])
            if finish_ep:
    ...
    def reset(self, seed=None):
        ...
        while not data or not dist:
            try:
                ...
                if isinstance(self, CarlaOvertaken) and self._retrain:
                    loc = self.ego_vehicle.get_location()
                    loc_front = self._front_vehicle.get_location()
...
class CarlaOvertaken(CarlaBase):
    ...
    def reset(self, seed=None):
        ...
        self._error_lane = 40
        self._error_crash = 60
        self._throttle_overtaken = -1
        self._throttle = 0
        self._counter_seen = 0 # Factor de tiempo en la recompensa
        self._seen = False
        self._array_throttle = []
    ...
    def _get_info(self):
        ...
        info[KEY_THROTTLE] = self._throttle
        return info
    ...
    def _get_control(self, action:np.ndarray):
        ...
        '''
        if self._overtaken_in_progress and self._throttle > self._throttle_overtaken and not self._train and self._back_lidar:
            print("Truncando el acelerador a:", self._throttle_overtaken, "|| era", self._throttle)
            self._throttle = self._throttle_overtaken
        '''
    ...
    def step(self, action:np.ndarray):
        ...
        try:
                ...
                if not self._change_lane_left and not self._change_lane_right and num_carriles > 1:#self._camera.check_lane_left():
                    ...
                    if self._overtaken_in_progress != self._per_over:
                        if self._overtaken_in_progress:
                            if not self._train:
                                arr = np.array(self._array_throttle)
                                self._throttle_overtaken = np.mean(arr)
                                print(f"\033[91mEMPIEZA EL ADELANTAMIENTO, throttle {self._throttle_overtaken}\033[0m")
                            else:
                            ...
                            self._seen = False
                            #self._counter_seen = 0 # NO RESETEAMOS
            ...
            terminated = True
            error = str(e)
            print("Circuit", self._id, "not completed:", error)
        ...
        if finish_ep:
            print("Circuit", self._id, "completed!")
        ...
        if terminated and self._train:
            ...
            self._writer_csv_train.writerow([self._count_ep, self._total_reward, self._count, finish_ep,
                                             ...
                                             finish_ep and not self._change_lane_right])
    ...
    def _calculate_reward(self, error:str):
        ...
        if error == None:
            ...
            if self._overtaken_in_progress and not self._change_lane_left:
                '''
                if self._dev < 0:
                    print("lado contrario:", np.clip(self._dev, -self._error_lane, 0) , "dev", self._dev)
                    return np.clip(self._dev, -self._error_lane, 0) 
                '''
                ...
                '''
                if self._dev > 0:
                    print("lado contrario:", -np.clip(self._dev, 0, self._error_lane), "dev", self._dev)
                    return -np.clip(self._dev, 0, self._error_lane)
                '''
                # Negtaive deviation
            ...
            # Counter seen normalizacion
            r_seen = (MAX_COUNTER_SEEN - np.clip(self._counter_seen, 0, MAX_COUNTER_SEEN)) / MAX_COUNTER_SEEN
            ...
            if r_steer == 0:
                ...
                if not self._train and not self._change_lane_left:
                    self._array_throttle.append(self._throttle)
            else:
                w_dev = 0.75
                w_throttle = 0.05
                w_steer = 0.2
            ...
            reward = w_dev * r_dev + w_throttle * r_throttle + w_steer * r_steer + w_laser * r_laser + w_seen * r_seen
        else:
